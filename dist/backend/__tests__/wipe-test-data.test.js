...
const wipeScript = path_1.default.join(repoRoot, "scripts", "wipe-test-data.ts");
// ---------------------------------------------------------------------------
...
function readConfiguredDatabasePath() {
    ...
    return path_1.default.isAbsolute(filePath) ? filePath : path_1.default.resolve(schemaDir, filePath);
}
/**
 ...
 */
function prepareTestDb(dbPath) {
    const canonicalDbPath = readConfiguredDatabasePath();
    assert(fs_1.default.existsSync(canonicalDbPath), `Configured DB does not exist: ${canonicalDbPath}`);
    fs_1.default.copyFileSync(canonicalDbPath, dbPath);
 ...
 */
function makePrismaClient(databaseUrl) {
 ...
 */
function runWipeScript(databaseUrl, args) {
    // shell: true es necesario en Windows para resolver npx desde PATH
    const result = (0, child_process_1.spawnSync)("npx ts-node " + [wipeScript, ...args].join(" "), [], {
        cwd: repoRoot,
...
async function test3_noFlagsAborts() {
    ...
    try {
        ...
        assert(combined.includes("--dry-run") || combined.includes("--confirm"), `stderr/stdout debe mencionar --dry-run o --confirm, got: ${combined}`);
...
async function test4_deleteOrderRespectsRestrictFks() {
    ...
    try {
        ...
        assert(result.status === 0, `exit code debe ser 0 si el orden es correcto, got ${result.status}\nstderr: ${result.stderr}\nstdout: ${result.stdout}`);
        assert((await prisma.product.count()) === 0, "Product debe quedar en 0");
        assert((await prisma.category.count()) === 0, "Category debe quedar en 0");
        assert((await prisma.stockMovement.count()) === 0, "StockMovement debe quedar en 0");
...
async function test5_emptyDbIsIdempotent() {
    ...
    try {
        ...
        await prisma.supplier.deleteMany();
        const result = runWipeScript(databaseUrl, ["--confirm"]);
        assert(result.status === 0, `exit code debe ser 0 con DB vacía, got ${result.status}\nstderr: ${result.stderr}`);
...
//# sourceMappingURL=wipe-test-data.test.js.map