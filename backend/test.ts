...
async function main() {
  ...
  try {
    ...
    const adjustmentById = await getStockMovementById(movementAdjustment.id)
    ...
    await deleteStockMovement(movementAdjustment.id, true)
    const afterRevertAdjustment = await getProductById(product.id)
    ...
    const adjustmentAfterDelete = await getStockMovementById(movementAdjustment.id)
    ...
    await updateProductStock(product.id, 2)
    const afterReset = await getProductById(product.id)
    assert(afterReset?.stock === 2, "precondition: stock reset to 2 before negative-flow tests")

    const negativeDelta = await changeProductStock(product.id, -5)
    ...
    const negativeSale = await createStockMovement({
      productId: product.id,
      type: "SALE",
      quantity: 4,
      notes: "oversell test"
    })
    const afterNegativeSale = await getProductById(product.id)
    ...
    await updateProductStock(product.id, 2)
    const inToRevert = await createStockMovement({
      productId: product.id,
      type: "IN",
      quantity: 3,
    ...
    await updateProductStock(product.id, 1)
    await deleteStockMovement(inToRevert.id, true)
    const afterRevertIn = await getProductById(product.id)
    ...
    await updateProductStock(product.id, 10)
    ...
    const stockAfterSale1 = await getProductById(product.id)
    assert(stockAfterSale1?.stock === 8, `createSale should decrement stock from 10 to 8, got ${stockAfterSale1?.stock}`)

    const sale1Movements = await listStockMovements({ productId: product.id, type: "SALE" })
    ...
    const stockAfterSale2 = await getProductById(product.id)
    assert(stockAfterSale2?.stock === 4, `createSale should decrement stock by sum of quantities (8 - 4 = 4), got ${stockAfterSale2?.stock}`)
    const sale2Movements = await listStockMovements({ productId: product.id, type: "SALE" })
    ...
    let paymentMismatchError: Error | null = null
    try {
      await createSale({
    ...
    let invalidMethodError: Error | null = null
    try {
      await createSale({
        items: [{ productId: product.id, quantity: 1, unitPrice: 100 }],
    ...
    assert(
      invalidMethodError!.message.includes("Invalid sale payment method"),
      `Expected invalid-method error, got ${invalidMethodError!.message}`
    )
    logStep("createSale rejects invalid payment method")

    let emptyItemsError: Error | null = null
    try {
      await createSale({
        items: [],
    ...
    assert(
      emptyItemsError!.message.includes("at least one item"),
      `Expected empty-items error, got ${emptyItemsError!.message}`
    )
    logStep("createSale rejects empty items")

    const saleNegative = await createSale({
      items: [{ productId: product.id, quantity: 10, unitPrice: 10 }],
      payments: [{ method: "CASH", amount: 100 }],
      total: 100
    })
    const afterNegativeStock = await getProductById(product.id)
    ...
    const missingSale = await getSaleById(-1)
    ...
    const salesByProduct = await listSales({ productId: product.id })
    assert(
      salesByProduct.some((s) => s.id === sale1.id) &&
        salesByProduct.some((s) => s.id === sale2.id) &&
        salesByProduct.some((s) => s.id === saleNegative.id),
      "listSales should filter sales that include the given productId"
    )
    logStep("listSales filter by productId")

    const salesByTransfer = await listSales({ method: "TRANSFER" })
    ...
    const recentSales = await listSales({ fromDate: cutoffDate })
    ...
    const oldSales = await listSales({ toDate: cutoffDate })
    ...
    const product1AfterSale3 = await getProductById(product.id)
    assert(
      product1AfterSale3?.stock === stockProductBeforeMulti - 1,
      `multi-product sale should decrement product1 stock by 1, expected ${stockProductBeforeMulti - 1}, got ${product1AfterSale3?.stock}`
    )
    const product2AfterSale3 = await getProductById(product2.id)
    assert(product2AfterSale3?.stock === 3, `multi-product sale should decrement product2 stock (5 - 2 = 3), got ${product2AfterSale3?.stock}`)
    const product2SaleMovements = await listStockMovements({ productId: product2.id, type: "SALE" })
    assert(
      product2SaleMovements.some((m) => m.saleId === sale3.id),
      "multi-product sale should create a StockMovement for product2 linked via saleId"
    )
    logStep("createSale with multiple distinct products")

    const saleLowercase = await createSale({
      items: [{ productId: product.id, quantity: 1, unitPrice: 100 }],
      payments: [{ method: "cash", amount: 100 }],
      total: 100
    })
    assert(
      saleLowercase.payments[0].method === "CASH",
      `createSale should normalize payment method to uppercase, got ${saleLowercase.payments[0].method}`
    )
    logStep("createSale normalizes payment method case")

    let totalMismatchError: Error | null = null
    try {
      await createSale({
    ...
    const salesByProduct2 = await listSales({ productId: product2.id })
    assert(salesByProduct2.some((s) => s.id === sale3.id), "listSales productId=product2 should include sale3")
    assert(
      !salesByProduct2.some((s) => s.id === sale1.id),
      "listSales productId=product2 should exclude sales that only reference product1"
    )
    ...
    await prisma.sale.deleteMany({ where: { items: { some: { productId: product.id } } } })
    await prisma.sale.deleteMany({ where: { items: { some: { productId: product2.id } } } })
    await prisma.stockMovement.deleteMany({ where: { productId: product.id } })
